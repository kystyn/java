...
#### Интерфейсы
- Могут содержать:
    - абстрактные методы - без реализации, должны быть определены (@Override) в классах-implementetor'ах;
    - default методы - с реализацией, могут быть как переопределены, так и не переопределены, без экземпляра использование невозмножно;
    - static методы - c реализацией, не могут быть переопределены, можно использовать без создания экземпляра интерфейса;
    - константы.
 - В отличие от классов допускается множественное наследование интерфейсов (extends).
 - Один класс может реализовать сразу множество интерфейсов (implements).
 - Все методы интерфейса public abstract.

- Создание экземпляра интерфейса в чистом виде без классов - по сути создание анонимного класса, переопределяющего абстрактные методы:
    - CE - Что за CTOR вызывается?
    ```
    SomeInterface si = new SomeInterface();
    ```
    - Нужно прямо на лету подложить реализацию методов, требующих (пере)определения:
    ```
    SomeInterface si = new SomeInterface{
        /*реализация обещанных по контракту методов*/
        void foo(){
            /*тело*/
        };
    };
    //теперь так делать легально:
    si.foo();
    ```
    #### @FunctionalInterface
    - Интерфейс, содержащий только один! абстрактный метод.
    - (C Java8 допускается наличие помимо основного абстрактного еще default методов, static методов и методов класса Object).
    - Создание экземпляра функционального интерфейса на лету такое же, как и для обычных интерфейсов. В этом случае быстро в нужном месте кода имеем экземпляр типа данных с единственным методом с вполне конкретным поведением, семантикой и смыслом.
#### Абстрактные классы
    TODO
