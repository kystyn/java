...
- Итак, `Manager` знакомит работников с `R` и `W`, выставляя их на конвейер, а также создает для каждого `E`, `W` и `R` свой ```Thread thr = new Thread(E);``` и запускает его. 
